...
6. REGRAS RIGOROSAS DE FORMATAÇÃO E IMAGENS: Ao ler a prova, se identificar uma figura, gráfico ou imagem no meio do texto, insira EXATAMENTE o marcador [IMAGEM_N] no local exato da aparição. O índice N corresponde à posição na lista fornecida (Imagem 1 → [IMAGEM_0], etc.).
    ...
    -MÚLTIPLOS TEXTOS DE APOIO: Quando a questão contiver "Texto I", "Texto II", "Figura 1", "Figura 2", preste atenção redobrada à ordem. A imagem do Texto II deve ficar ESTRITAMENTE após o conteúdo do Texto II e antes da próxima pergunta. Siga a leitura de cima para baixo.

    7. RECONSTRUÇÃO DE TABELAS EM HTML: O OCR pode ter desestruturado as tabelas presentes no PDF. Se você identificar visualmente que existe uma tabela de dados na prova original, NÃO retorne o texto bagunçado. Você DEVE reconstruir a tabela utilizando marcação HTML padrão (<table>, <thead>, <tbody>, <tr>, <th>, <td>).

Insira a tabela HTML reconstruída diretamente no fluxo do texto_anotado, no local exato onde ela aparece no PDF.

Não use Markdown (|---|---|), use APENAS tags HTML válidas para as tabelas.
